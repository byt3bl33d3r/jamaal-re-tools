...
class EthDisplayControl(taskmods.DllList):
    ...
    def run_config(self):
        ...
        if self.config.FILTER_PACKET:
            ...
            for filtername in  temp_list:
                ...
                # // Rut Roh
                print "debug wtf", len(self.ethkeylist or self.protokeylist)
    ...
    def buildpkt_string(self):
        ...
        plist = sorted(self.tmppkt.items())

        # // Packet Table 
        #0    ('ethDst', 'ff:ff:ff:ff:ff:ff')   [0][0]            [0][1]
        #1    ('ethSrc', '00:50:56:c0:00:08')   [1][0]            [1][1]
        #2    ('ethType', 2048L)   [2][0]            [2][1]
        #3    ('ipv4Checksum', 18819L)   [3][0]            [3][1]
        #4    ('ipv4DSF', 0L)   [4][0]            [4][1]
        #5    ('ipv4Dest', '172.16.176.255')   [5][0]            [5][1]
        #6    ('ipv4DstPort', '35072')   [6][0]            [6][1]
        #7    ('ipv4FLAG', 0L)   [7][0]            [7][1]
        #8    ('ipv4IDENT', 30714L)   [8][0]            [8][1]
        #9    ('ipv4ProtoType', 17L)   [9][0]            [9][1]
        #10    ('ipv4Source', '172.16.176.1')   [10][0]            [10][1]
        #11    ('ipv4SrcPort', '35054')   [11][0]            [11][1]
        #12    ('ipv4TTL', 64L)   [12][0]            [12][1]
        #13    ('ipv4TotalLen', 78L)   [13][0]            [13][1]
        #14    ('ipv4Ver', 69L)   [14][0]            [14][1]
        #15    ('pdata', 'E\x00\x00Nw\xfa\x00\x00')  [15][0]            [15][1]
        #16    ('pheader', '\xff\xff\xff\xff\')   [16][0]            [16][1]

        ethname,ethnum, ethnumstr  = self.pdiddy.get_ethtype(plist[2][1])
        protostr,protonum, protonumstr = self.pdiddy.get_prototye(plist[9][1])
        source = self.tmppkt.get(plist[10][0])
        srcport = self.tmppkt.get(plist[11][0])
        macsrc = self.tmppkt.get(plist[1][0])
        dst = self.tmppkt.get(plist[5][0])
        dstport = self.tmppkt.get(plist[6][0])
        macdst = self.tmppkt.get(plist[0][0])
        pdata = self.tmppkt.get(plist[15][0])
        pheader = self.tmppkt.get(plist[16][0])
        
        pktstring = "Ethernet Type: " + ethname + " (" + ethnumstr + ") " + "\n"
        pktstring += "Protocol " +protostr + " " + "("+str(protonum)+")" + "\n"
        pktstring += "Src: " + source +":" + srcport + " (" +macsrc+"), " + "Dst: " + dst +":" + dstport + " (" +macdst+")" +"\n\n"
        pktstring += "Full Packet" + "(" + str(len(pheader+pdata)) + " Bytes" +")" + "\n" 
    ...
    def get_packet_process(self, outfd, pktoffset, ptpobj):
...
class EthScan(common.AbstractWindowsCommand):
    ...
    def __init__(self, config, *args, **kwargs):
        ...
        config.add_option("FILTER-PACKET", short_option= 'F',  default=None,  type="string",
                     ...
                     'Example: " -F UDP,TCP,IPV6 " - searches only for UDP, TCP and IPv6 type packets.') # << FIX THIS WITH CORRECT OPTIONS
    ...
    def render_text(self, outfd, data):
        ...
        for objct in data:
            buildpkt = self.ethcontrol.buildpkt(objct)
